...
namespace isl {
  ...
  public:
    ...
    virtual bool hasTupleId(isl_dim_type type) const = 0;
    virtual Id getTupleId(isl_dim_type type) const = 0;
    virtual void setTupleId(isl_dim_type type, Id &&id) = 0;

    virtual bool hasTupleName(isl_dim_type type) const = 0;
    virtual const char *getTupleName(isl_dim_type type) const = 0;
    virtual void setTupleName(isl_dim_type type, const char *s) = 0;

    virtual bool hasDimId(isl_dim_type type, unsigned pos) const = 0;
    ...
    virtual int findDimById(isl_dim_type type, const Id &id) const  = 0;

    virtual bool hasDimName(isl_dim_type type, unsigned pos) const  = 0;
    virtual const char *getDimName(isl_dim_type type, unsigned pos) const  = 0;
    virtual void setDimName(isl_dim_type type, unsigned pos, const char *s)  = 0;
    virtual int findDimByName(isl_dim_type type, const char *name) const = 0;
