...
#pragma endregion
    ...
    void add_inplace(const PwAff &paff) ISLPP_INPLACE_FUNCTION { give(isl_pw_aff_list_add(take(), paff.takeCopy())); }
    PwAffList add(const PwAff &paff) const { return PwAffList::enwrap(isl_pw_aff_list_add(takeCopy(), paff.takeCopy())); }

    void insert_inplace(pos_t pos, const PwAff &paff) ISLPP_INPLACE_FUNCTION { give(isl_pw_aff_list_insert(take(), pos, paff.takeCopy())); }
    ...
    void drop_inplace(unsigned first, unsigned count) ISLPP_INPLACE_FUNCTION { give(isl_pw_aff_list_drop(take(), first, count)); }
    PwAffList drop(unsigned first, unsigned count) const { return PwAffList::enwrap(isl_pw_aff_list_drop(takeCopy(), first, count)); }

    void concat_inplace(const PwAffList &rhs) ISLPP_INPLACE_FUNCTION { give(isl_pw_aff_list_concat(take(), rhs.takeCopy())); }
    ...
    void setPwAff_inplace(pos_t index, PwAff &paff) ISLPP_INPLACE_FUNCTION { give(isl_pw_aff_list_set_pw_aff(take(), index, paff.takeCopy())); }
    ...
    void sort_inplace(const std::function<int(const PwAff&, const PwAff&)> &func) ISLPP_INPLACE_FUNCTION;
  ...
  static inline PwAffList concat(const PwAffList &lhs, const PwAffList &rhs) ISLPP_INPLACE_FUNCTION { return PwAffList::enwrap(isl_pw_aff_list_concat(lhs.takeCopy(), rhs.takeCopy())); }
