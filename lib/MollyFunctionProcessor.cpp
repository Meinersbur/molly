...
class MollyFunctionContext : public MollyFunctionProcessor, private FunctionPass, private AnalysisResolver {
private:
  MollyPassManager *pm;
  Function *func;
...
public:
  static char ID;
  MollyFunctionContext(MollyPassManager *pm, Function *func): FunctionPass(ID), pm(pm), func(func), AnalysisResolver(*static_cast<PMDataManager*>(nullptr)) {
    assert(pm);
    assert(func);
...
#pragma region molly::MollyFunctionProcessor
  ...
  virtual llvm::AnalysisResolver * asResolver() LLVM_OVERRIDE {
    return this;
  }
#pragma endregion
