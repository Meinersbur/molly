...
namespace {
  ...
  public:
    ...
    void init(int &argc, char **(&argv)) {
      ...
      _cart_self_coords = static_cast<int*>(malloc(sizeof(*_cart_self_coords) * _cart_dims)); 
      MPI_CHECK(MPI_Cart_coords(_cart_comm, _cart_self, __molly_cluster_dims, _cart_self_coords));
    ...
    int getSelfCoordinate(uint64_t d) {
      assert(initialized);
      assert(_cart_self_coords);
      assert(0 <= d && d < _world_ranks);
      return _cart_self_coords[d];
    }
  }; // class MPICommunicator
...
extern "C" int __molly_main(int argc, char *argv[], char *envp[]/*, uint64_t expectedClusterDims, uint64_t *expectedClusterShape*/) {
...
extern "C" void __molly_combuf_send_dst_init(MPISendCommunication *sendbuf, uint64_t dst, uint64_t count) {
